...
def render():
    ...
    st.markdown("""
    <div class="metric-card" style="padding: 16px 24px; margin-bottom: 24px;">
        <h4 style='margin-top: 0; color: var(--primary); font-size: 0.95rem; text-transform: uppercase; letter-spacing: 0.05em; font-weight: 700;'>Filters</h4>
    ...
    st.markdown(f"<p style='color: var(--text-muted);'>Found <strong style='color: var(--primary); font-size: 1.1rem;'>{len(filtered_df):,}</strong> claims matching criteria.</p>", unsafe_allow_html=True)
    
    # Style the dataframe container
    st.markdown("<div style='border: 1px solid var(--border); border-radius: 8px; overflow: hidden;'>", unsafe_allow_html=True)
